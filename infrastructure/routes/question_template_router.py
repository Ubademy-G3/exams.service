...
async def create_question_template(
    exam_id: str, question_template: QuestionTemplateSchema, db: Session = Depends(get_db), apikey: str = Header(None)
):
...
async def get_question_template(
    exam_id: str, question_template_id: str, db: Session = Depends(get_db), apikey: str = Header(None)
):
    auth_service.check_api_key(apikey)
    question_template_list = QuestionTemplateController.get_question_template(db, exam_template_id)
    return {
...
async def get_all_question_templates_by_exam_template_id(
    exam_template_id: str, db: Session = Depends(get_db), apikey: str = Header(None)
):
    auth_service.check_api_key(apikey)
